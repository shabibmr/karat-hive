...
| **Version** | 1.5 |
| **Status** | Draft — PO walk decisions applied (§1–3, §5, §9) + technology stack fixed (§2.4, §2.5); C-10 and C-13 resolved (§2.5, §7.6, §9.7); ornament type and purity relaxed to optional on *Find An Ornament* (`FR-CUS-006`) |
| **Date** | 26 September 2026 |
| **Supersedes** | `docs/old/Requirements-Spec-v1.3.md` (v1.3), `docs/old/Requirements-Spec-v1.2.md` (v1.2), `docs/old/Requirements-Spec-v1.1.md` (v1.1), `docs/old/Requirements-Spec.md` (v1.0) |
...
3. Ornament type (ring, chain, bangle, necklace, earring, bracelet, pendant, other) and purity are optional — the reference image and notes may be all the Customer has to hand. `[ASSUMED]`
...
The Vendor shall be able to declare the Regions they serve (as an optional feed filter) and their business hours/availability. Category selection is removed, and Vendor activation depends unconditionally on VERIFIED status.
...
stateDiagram-v2
    ...
    VERIFIED --> ACTIVE: Unconditional upon verification (FR-VEN-025)
...
Additionally, individual acceptance criteria carrying an inline `[ASSUMED]` tag appear within `FR-CUS-005`, `FR-CUS-006`, `FR-CUS-007`, `FR-CUS-009`, `FR-CUS-010`, `FR-CUS-029`, `FR-VEN-003`, `FR-VEN-011`, `FR-VEN-012`, `FR-VEN-023`, `FR-VEN-028`, `FR-ADM-001`, and `FR-ADM-026`. These are specific parameter choices — thresholds, limits, durations — within otherwise sourced requirements, and warrant confirmation of the values rather than of the requirement itself.
...
> **Sign-off progress (v1.4):** §9.5 commercial decisions and the §1–3 / §5 / §9 PO walk are **resolved**. The technology stack is fixed and all four constraints C-10–C-13 are now resolved (§9.7), so **Technical Lead sign-off has something concrete to sign against**. Remaining: Legal review of Yahoo Finance redistribution and ToS liability wording; **object-storage data residency for KYC personal data under `NFR-020`** (Infrastructure, non-blocking — swappable behind the S3 adapter); the Admin data-grid build-or-buy decision (`AD-FE-12`); the new `FR-CUS-006` AC3 `[ASSUMED]` tag (ornament type/purity now optional); and the remaining Appendix B.3 `[ASSUMED]` items not decided in the PO walk (e.g. media limits, notification edge cases), which should be triaged before build freeze.
