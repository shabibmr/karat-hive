...
**Current tree (7 Sep 2026, orchestrator pass):** `CM-S01`–`S05`, `S10` committed; `S11` on disk uncommitted; `S03`/`S04` domain files verified (`offer.dart`, `connection.dart`, config/gold-rate/settings/review/notification/abuse). `CM-S06` remains **partial** (`KhApi` still parses config/rates/settings via `kh_api` DTOs). Gate **S** is **not** passed (`S07`–`S09`, `S12` open). Track K unchanged.
...
**Counts this pass:** 7 **done** · 1 **partial** · 54 **open** · 0 **blocked**.
...
| **Ticked 7 Sep (v0.3)** | done **7** · partial **1** · open **54** · blocked **0** | | Orchestrator pass after domain S03/S04 |
