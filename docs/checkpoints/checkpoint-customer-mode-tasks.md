...
**Current tree (7 Sep 2026, orchestrator pass):** Wave 1 landed `CM-S01`/`S02` (committed), `CM-S10` (committed), `CM-S05`/`S11` (on disk, uncommitted), `CM-S06` (on disk, maps to `kh_api` DTOs not `kh_domain` — **partial**). `kh_mobile` shells remain Vendor-only. Gate **S** is **not** passed (`S03`–`S04`, `S07`–`S09`, `S12` still open). Backend Customer routes remain assumed contract. Track K is unchanged.
...
| CM-S03 | open | No `offer.dart` / `connection.dart`. Stray untracked `offer_connection_test.dart` only |
| CM-S04 | open | `PlatformConfig` / `GoldRate` / `Settings` live in `kh_api` DTOs, not `kh_domain` |
| ~~CM-S05~~ | **done** (uncommitted) | AED/g/karat/GST/relative-time formatters + Customer EN/AR keys |
| CM-S06 | **partial** | `registerCustomer`, `patchMe`, platform-config, gold-rates, settings methods exist; they parse `kh_api` DTOs, not `kh_domain` (`S04`) |
| CM-S07–S09 | open | Not started |
| ~~CM-S10~~ | **done** | `MediaUploader` + `KYC_DOCUMENT` / `REQUEST_IMAGE`; Vendor KYC uses helper; tests |
| ~~CM-S11~~ | **done** (uncommitted) | `KhNumericField`, `KhSelectField`, `KhConfirmDialog`, `KhBadge`, `KhStatusChip` + `foundations_test` |
| CM-S12 | open | No Customer shell / bottom-nav / pull-to-refresh widgets |
...
**Counts this pass:** 5 **done** · 1 **partial** · 56 **open** · 0 **blocked**.
...
| **Ticked 7 Sep (v0.2)** | done **5** · partial **1** · open **56** · blocked **0** | | Orchestrator pass after Wave 1 |
