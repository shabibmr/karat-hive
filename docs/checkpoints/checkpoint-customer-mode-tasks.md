...
**Current tree (7 Sep 2026, orchestrator pass):** Wave 1 landed `CM-S01`/`S02`/`S05`/`S10` (committed), `CM-S11` (on disk, uncommitted), `CM-S06` (on disk, maps to `kh_api` DTOs not `kh_domain` — **partial**). `kh_mobile` shells remain Vendor-only. Gate **S** is **not** passed (`S03`–`S04`, `S07`–`S09`, `S12` still open). Backend Customer routes remain assumed contract. Track K is unchanged.
