...
| Screen | Load / list | Actions | Empty / error |
|---|---|---|---|
| **ADM-S01** Login with 2FA | *client* | `POST /v1/auth/login/password` → `POST /v1/auth/admin/2fa/verify` | `→ TWO_FACTOR_REQUIRED` / `TWO_FACTOR_INVALID` · `→ ACCOUNT_LOCKED` (3 / 30 min) |
...
| **ADM-S14** Category management | `GET /v1/admin/categories` (incl. inactive) | `POST /v1/admin/categories` · `PATCH /v1/admin/categories/{id}` · `POST /v1/admin/categories/{id}/deactivate` | "Delete" in use `→ TAXONOMY_IN_USE` — but there is no DELETE route ⚠️ `SAM-GAP-9` |
| **ADM-S15** Region management | `GET /v1/admin/regions` | `POST /v1/admin/regions` · `PATCH /v1/admin/regions/{id}` · `POST /v1/admin/regions/{id}/deactivate` | as ADM-S14 ⚠️ `SAM-GAP-9` |
...
| **ADM-S20** Gold rate configuration | `GET /v1/admin/gold-rates` · `GET /v1/admin/gold-rates/history` | `POST /v1/admin/gold-rates/override` · poll interval / staleness = `PATCH /v1/admin/settings/{key}` ⚠️ `SAM-GAP-11` | feed down → `gold-rates` health block · override active → shown in payload |
...
| `SAM-GAP-9` | L | ADM-S14, ADM-S15 | Screen files list a **Delete** action ("blocked if in use"); Route Inventory §21.6 states "There is no DELETE" (deactivate only). Not a missing endpoint — a wording mismatch that will confuse implementers. | Drop "Delete" from the two screen files (keep "Deactivate"), or note explicitly that Delete is disabled and always resolves to `→ TAXONOMY_IN_USE` / deactivate. |
...
| `SAM-GAP-11` | L | ADM-S20 | Screen presents "feed refresh interval" and "staleness threshold" as editable fields on the gold-rate screen; they are actually `PLATFORM_SETTING` keys edited via `PATCH /v1/admin/settings/{key}`. Functional, but the screen implies a dedicated control. | Document on the screen that these two fields write through the settings endpoint; no new route needed. |
