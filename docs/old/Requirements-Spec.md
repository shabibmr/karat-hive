...
| **Version** | 1.0 |
| **Status** | **Superseded** by `docs/Requirements-Spec-v1.1.md` |
| **Date** | 10 August 2026 |
| **Source** | `docs/Requirements-raw.txt` |
...
**In scope for v1.0:** account management for all three actors, vendor verification, the four Request types, offer submission and comparison, offer acceptance and identity reveal, WhatsApp handoff, two-way ratings, notifications, and a full administrative back-office.

**Out of scope for v1.0** — see §9.4 for the complete list with rationale. In summary: no in-app payments or escrow, no logistics or delivery tracking, no in-app messaging beyond the WhatsApp handoff, and no jewellery assaying or authentication service.
...
Karat Hive is a **new, self-contained system** with no predecessor and no requirement to integrate with an existing platform. It comprises four deployable components:

| Component | Users | Platform |
|---|---|---|
| **Customer App** | Customers | Native / cross-platform mobile (iOS + Android) |
| **Vendor App** | Vendors / Jewellers | Native / cross-platform mobile (iOS + Android) |
| **Admin Portal** | Platform Admins | Responsive web application |
| **Backend Platform** | — | API services, datastore, object storage, job scheduler, notification gateway |

The Customer and Vendor experiences are delivered as **two distinct mobile applications**, not as two modes of one application. Their workflows, information density, and session patterns differ materially: a Customer opens the app occasionally to post a Request and review Offers; a Vendor uses it as a working tool, monitoring an inbound request feed throughout the business day.
...
| Aspect | Requirement |
|---|---|
| Customer & Vendor apps | iOS 15+ and Android 9 (API 28)+ |
| Admin Portal | Current-and-previous major versions of Chrome, Edge, Safari, and Firefox; responsive from 1280 px upward |
...
| Backend | Cloud-hosted; region selected to satisfy UAE data-residency expectations (see `NFR-020`) |

### 2.5 Design and Implementation Constraints

| ID | Constraint |
|---|---|
| C-01 | The transaction currency is **AED**. All monetary values are stored and displayed in AED. |
...
| C-04 | Only Vendors in the `VERIFIED` **and** `ACTIVE` state may view or respond to Requests (`BR-002`). |
...
Consolidated in §9. The dependencies with the greatest delivery risk are the **WhatsApp deep-link handoff** (§7.2), the **push notification gateway** (§7.3), and the **gold spot-rate feed** (§7.4) — the last of which is `[ASSUMED]` and not present in the source notes.
...
**Behaviour.** Low session frequency, high intent. Opens the app with a specific need, creates a Request, then returns intermittently over the following hours or days as Offers arrive. Price-sensitive and comparison-driven. Wary of unsolicited contact.

**Account lifecycle:** `REGISTERED → ACTIVE → (SUSPENDED) → (DEACTIVATED)`

**Verification.** None required. A Customer transacts as an individual and is not vetted by the platform.
...
**Verification.** Mandatory. A Vendor submits business credentials and is manually reviewed and approved by a Platform Admin (`FR-ADM-015`) before gaining any access to Requests.
...
#### FR-CUS-001 · Customer Registration `[ASSUMED]`
**Priority:** Must · **Source:** `[ASSUMED]` — the raw notes describe Customer activity but never specify how a Customer account comes into existence.

The system shall allow a prospective Customer to self-register using a UAE-format mobile number verified by one-time password (OTP), capturing name and mobile number as mandatory fields and email as optional.
...
4. The Customer receives an on-screen confirmation showing the Request reference and the expiry date.
...
5. A submitted review is published to the Vendor's public profile after passing moderation (`FR-ADM-026`).
...
**Acceptance criteria**
1. Notification triggers: first Offer received on a Request; each subsequent Offer received; Request approaching expiry (24 h before); Request expired; Offer withdrawn or revised by a Vendor; review reminder; platform announcement.
...
4. On submission the account enters `PENDING_VERIFICATION`; the Vendor may log in and complete their profile but **cannot view or respond to any Request** until verified (`BR-002`).
...
**Priority:** Must · **Source:** raw §2.1 (L38)

The system shall authenticate a Vendor and maintain their authenticated session.

**Acceptance criteria**
1. Authentication is by registered mobile number and OTP, or by email and password where the Vendor has set one.
2. Login is permitted in states `PENDING_VERIFICATION`, `VERIFIED`, and `ACTIVE`; it is refused in `REJECTED`, `SUSPENDED`, and `DEACTIVATED` with a state-appropriate message.
3. A Vendor in `PENDING_VERIFICATION` lands on a status screen, not the dashboard.
4. Sessions expire after 14 days of inactivity — shorter than the Customer session, reflecting the higher sensitivity of Vendor data. `[ASSUMED]`
5. Five consecutive failed authentication attempts lock the account for 15 minutes and notify the registered contact.
...
**Acceptance criteria**
1. The Vendor selects a validity period at submission from a configured set (default options: 24 h, 48 h, 72 h, 7 days), defaulting to 48 hours.
...
4. The Vendor is notified 6 hours before their Offer expires and may extend it once, by up to the original validity period.
...
3. Customer ratings are aggregated and surfaced to Vendors as a trust signal on future Requests from that Customer (`FR-VEN-010`), and are **never** displayed publicly to other Customers. `[ASSUMED]`
...
3. The Vendor may flag a review as unfair, which routes it to the Admin moderation queue (`FR-ADM-026`) but does not remove it in the interim.
...
#### FR-VEN-031 · Vendor Subscription and Offer Quota `[ASSUMED]`
**Priority:** Could · **Source:** `[ASSUMED]` — the raw notes describe no revenue model at all. This requirement is included so that monetisation is an explicit, reviewable decision rather than an omission discovered late.

The system shall support subscription tiers that govern a Vendor's monthly Offer allowance and feature access.

**Acceptance criteria**
1. Tiers, their allowances, and their prices are configured by an Admin (`FR-ADM-030`).
2. A Vendor's remaining allowance for the current period is displayed on the dashboard, and Offer submission is blocked once exhausted, with an upgrade path presented.
3. A free tier with a limited monthly allowance exists, so that a newly verified Vendor can transact immediately.
4. Tier changes take effect at the start of the next billing period, except upgrades, which take effect immediately.

> **Product Owner decision required.** This requirement presumes a subscription revenue model. The alternatives — commission per Connection, lead fees, or a fully free platform — imply materially different requirements. Confirm or strike before build.
...
5. Approval is the sole event that grants a Vendor access to Requests (`BR-002`); no other action confers it.
...
**Priority:** Must · **Source:** raw §10 (L89)

The Admin shall be able to moderate reviews submitted by Customers and Vendors.

**Acceptance criteria**
1. A moderation queue lists reviews that are newly submitted, flagged by the reviewed party (`FR-VEN-029`), or auto-flagged by content filtering.
2. Available decisions: **Approve** (publish), **Reject** (withhold, with a reason sent to the author), and **Redact** (publish with the offending passage removed, retaining the original internally).
3. Rejecting or redacting a review triggers recomputation of the affected party's aggregate rating (`FR-SYS-012`).
4. Vendor responses to reviews (`FR-VEN-029`) pass through the same queue.
5. Every moderation decision is written to the audit log with the deciding Admin and rationale.
6. Reviews may be configured to publish immediately with post-hoc moderation, or to be held pending approval; the mode is a platform setting (`FR-ADM-030`). `[ASSUMED]`
...
**Acceptance criteria**
1. Configurable at minimum: Gold Bullion minimum value (`FR-CUS-013`); default and permitted Offer validity periods (`FR-VEN-013`); Request expiry period; maximum concurrent Requests per Customer; supported karat purities; image count and size limits; review moderation mode; and subscription tier definitions (`FR-VEN-031`).
...
#### FR-ADM-031 · Gold Rate Configuration `[ASSUMED]`
**Priority:** Should · **Source:** `[ASSUMED]` — the AED 500 bullion floor in raw §1.4 requires a rate, but no rate management is specified.

The Admin shall be able to configure the reference gold rate source and, where necessary, override the rate manually.

**Acceptance criteria**
1. Rate source is configurable between an external feed (§7.4) and manual entry.
2. Where a feed is configured, its refresh interval and staleness threshold are configurable.
3. A manual override may be applied per purity, requires a reason, and carries an explicit expiry after which the feed resumes.
4. Rate history is retained and viewable, so that any historical indicative valuation can be reconstructed and explained.
5. Rate changes are written to the audit log.
...
**Acceptance criteria**
1. Conditions, all of which must hold: verification state is `VERIFIED`; account state is `ACTIVE`; the Vendor's declared Categories include the Request's Category; the Vendor's served Regions include the Request's Region; and, where subscription quotas are enabled, remaining quota is non-zero (`FR-VEN-031`).
...
#### FR-SYS-005 · Request Expiry Processing `[ASSUMED]`
**Priority:** Must · **Source:** `[ASSUMED]` — the raw notes provide no Request lifetime, without which Requests accumulate indefinitely and the Vendor feed degrades.

The system shall automatically expire Requests that reach the end of their configured lifetime without an accepted Offer.

**Acceptance criteria**
1. The default Request lifetime is 30 days from publication, configurable per Request type (`FR-ADM-030`).
2. The Customer is notified 24 hours before expiry and offered a one-tap extension of a further 30 days, permitted at most twice.
...
#### FR-SYS-010 · Gold Rate Ingestion `[ASSUMED]`
**Priority:** Should · **Source:** `[ASSUMED]` — see `FR-CUS-018`, `FR-ADM-031`.

The system shall ingest reference gold rates from the configured source and make them available to rate-dependent features.

**Acceptance criteria**
1. Rates are fetched at the configured interval (default every 15 minutes) and stored with their source timestamp.
...
5. Sustained ingestion failure beyond 2 hours raises an Admin alert.
...
| ID | Rule | Enforced by |
|---|---|---|
| **BR-001** | A Customer account requires no verification; a Customer transacts as an individual. | `FR-CUS-001` |
| **BR-002** | A Vendor may view Requests and submit Offers **only** while in verification state `VERIFIED` **and** account state `ACTIVE`. | `FR-ADM-015`, `FR-SYS-002` |
...
stateDiagram-v2
    ...
    PUBLISHED --> EXPIRED: Lifetime reached (FR-SYS-005)
    ...
    OFFERS_RECEIVED --> EXPIRED: Lifetime reached
...
| `PUBLISHED` | Live, matched, awaiting first Offer | Yes |
| `OFFERS_RECEIVED` | Live with one or more pending Offers | Yes |
...
| `EXPIRED` | Lifetime elapsed without acceptance | No |
...
Only the `ACTIVE` state grants access to Requests (`BR-002`).
...
| `subscription_tier_id` | UUID | FK, nullable | `[ASSUMED]` — see `FR-VEN-031` |
...
| `expires_at` | timestamp | nullable | `FR-SYS-005` |
| `extension_count` | integer | default 0 | Max 2 |
...
|---|---|---|
| **Customer App** | iOS / Android | Optimised for infrequent, high-intent sessions. The Request creation flow is the critical path and must be completable in under 2 minutes for a returning user. Offer comparison must be legible on a phone screen without horizontal scrolling. |
| **Vendor App** | iOS / Android | Optimised for frequent, working-tool use. Information density is prioritised over whitespace. The Request feed must support rapid triage — scan, filter, open, offer — with the Submit Offer action reachable in at most two taps from the feed. |
| **Admin Portal** | Responsive web, ≥ 1280 px | Optimised for queue processing. Verification, moderation, and abuse queues must support keyboard-driven review without leaving the queue context. |

A complete screen inventory is given in **Appendix C**. All three interfaces support English and Arabic including full right-to-left layout (`NFR-022`).
...
### 7.4 Gold Rate Feed Interface `[ASSUMED]`
...
| Data required | Spot rate for 24K / 999 gold in AED per gram, with a source timestamp |
...
> **Commercial dependency, not yet resolved.** No rate provider is named in the source material. Provider selection, licensing cost, and permitted redistribution of rate data must be settled before this interface can be built. Whether the platform is contractually permitted to display a licensed rate to end users is a legal question, not a technical one.
...
| ID | Requirement |
|---|---|
| **NFR-007** | The platform shall support at least 10,000 concurrent active sessions and 500 requests per second at launch capacity, scaling horizontally beyond that without architectural change. |
| **NFR-008** | The data model and storage shall accommodate 500,000 Customers, 5,000 Vendors, 2,000,000 Requests, and 10,000,000 Offers without redesign. |
| **NFR-009** | Application tiers shall be stateless and horizontally scalable. Background processing — fan-out, expiry, notification dispatch, media processing — shall run as independently scalable workers, so that a spike in one does not degrade interactive request handling. |
...
| **NFR-011** | Data shall be backed up at least daily with point-in-time recovery over a 30-day window. Recovery Point Objective ≤ 1 hour; Recovery Time Objective ≤ 4 hours. Restore procedures shall be tested at least quarterly. |
...
| **NFR-023** | Mobile applications shall meet WCAG 2.1 Level AA (R5) as applicable to native mobile: minimum 4.5:1 text contrast, full screen-reader support (VoiceOver / TalkBack), 44×44 pt minimum touch targets, and support for OS-level dynamic type up to 200 % without layout breakage. The Admin Portal shall meet WCAG 2.1 AA including full keyboard operability. |
...
|---|---|---|
| A-01 | The market is the UAE and the currency is AED. | Multi-currency support and per-market regulatory work would be required. |
...
| A-03 | Vendor verification volume is low enough for manual Admin review to be sustainable. | An automated or outsourced verification pipeline would be needed. |
...
| A-05 | Vendors will monitor a request feed frequently enough to make the marketplace liquid. | Response times would lengthen and Requests would expire unanswered, requiring incentive mechanisms. |
| A-06 | A licensable gold rate feed is available on acceptable commercial terms. | `FR-CUS-018` and `FR-ADM-031` would reduce to manual Admin entry only. |
| A-07 | Off-platform settlement is acceptable to the business and does not attract payment-intermediary regulatory obligations. | Payments, escrow, and financial licensing would enter scope. |

### 9.2 Constraints

Recorded in §2.5 as C-01 through C-06.
...
| Gold rate provider | External, commercial | **High** — provider unselected; licensing terms unknown (§7.4) |
| APNs / FCM push | External | Medium — vendor push-token lifecycle and delivery reliability |
| SMS gateway (UAE) | External | Medium — OTP deliverability directly gates all registration |
| Cloud region with UAE residency | Infrastructure | Medium — constrains provider and region choice (`NFR-020`) |
| Apple App Store / Google Play review | Process | Medium — release cadence and the 6-month API support window (`NFR-027`) |
...
| Excluded | Rationale |
|---|---|
| **In-app payments, escrow, or settlement** | Settlement occurs off-platform (`BR-015`). Including it would attract payment-intermediary regulatory obligations and materially expand scope. |
...
| **Multi-currency and multi-country operation** | Constrained to AED and the UAE (C-01, A-01). |
...
### 9.5 Open Commercial Decisions

These are not technical gaps but business decisions that must be settled before or during build, because the requirements above depend on them.

| # | Decision required | Requirement affected |
|---|---|---|
| 1 | **Revenue model** — subscription, per-connection commission, lead fees, or free. `FR-VEN-031` presumes subscription. | `FR-VEN-031`, `FR-ADM-030` |
| 2 | **Gold rate provider** and the licensing terms for displaying its data to end users. | §7.4, `FR-CUS-018`, `FR-ADM-031` |
| 3 | **Review moderation posture** — publish-then-moderate, or hold-for-approval. | `FR-ADM-026` |
| 4 | **Verification service-level commitment** to Vendors — the turnaround the business will promise. | `FR-ADM-015` |
| 5 | **Liability position** on off-platform transactions, to be reflected in the Terms of Service. | `BR-015`, §9.4 |
| 6 | **Launch Region and Category coverage** — the minimum vendor density needed before opening to Customers in a given area. | `FR-SYS-002`, `FR-ADM-027` |
...
| **Customer** | A retail individual who creates Requests. Not verified by the platform. |
...
| **Request** | A Customer's stated intent to buy or sell, of one of four types. The originating entity of the value loop. |
...
| **Verification** | Manual Admin review of a Vendor's business credentials. The sole gate to Request access (`BR-002`, `BR-003`). |
...
**Coverage: 100 %.** All 81 non-blank lines of the source material are accounted for: 76 map to at least one requirement, and the remaining 5 are structural headings carrying no requirement content. No source item was dropped.
...
|---|---|---|
| `FR-CUS-001` | Customer Registration | The raw notes describe Customer activity but never account creation |
...
| `FR-VEN-031` | Subscription & Offer Quota | **No revenue model appears anywhere in the source** |
...
| `FR-SYS-005` | Request Expiry Processing | No Request lifetime defined; feed would degrade |
...
|---|---|---|
| `CUS-S01` | Onboarding — mobile entry, OTP verification | `FR-CUS-001`, `FR-CUS-002` |
...
| `VEN-S03` | Verification pending / rejected status | `FR-VEN-003`, `FR-ADM-015` |
...
| `VEN-S22` | Subscription & quota `[ASSUMED]` | `FR-VEN-031` |
...
|---|---|---|---|
| 1.0 | 10 Aug 2026 | Requirements Analysis | Initial SRS derived from `docs/Requirements-raw.txt`. 110 functional requirements, 22 business rules, 30 non-functional requirements. 37 requirements marked `[ASSUMED]` and awaiting Product Owner confirmation. |
...
> **Sign-off is conditional on the resolution of Appendix B.3** — the 37 `[ASSUMED]` requirements — and of the six open commercial decisions in §9.5.
