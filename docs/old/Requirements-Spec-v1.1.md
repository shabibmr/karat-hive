...
| **Version** | 1.1 |
| **Status** | Draft — PO walk decisions applied (§1–3, §5, §9) |
| **Date** | 10 August 2026 |
| **Supersedes** | `docs/old/Requirements-Spec.md` (v1.0) |
| **Source** | `docs/Requirements-raw.txt` + PO decisions 10 Aug 2026 |
...
|---|---|---|
| **Mobile App (dual-mode)** | Customers and Vendors / Jewellers | Native / cross-platform mobile (iOS + Android) — one binary, two modes |
...
| Aspect | Requirement |
|---|---|
| Dual-mode mobile app | iOS 15+ and Android 9 (API 28)+ — single install for Customer or Vendor mode |
...
| Backend | Cloud-hosted; region selected to satisfy UAE data-residency expectations for v1.0 (see `NFR-020`) |
...
|---|---|---|
| **Mobile app — Customer mode** | iOS / Android (shared dual-mode binary) | Optimised for infrequent, high-intent sessions. OAuth once before first publish. The Request creation flow is the critical path and must be completable in under 2 minutes for a returning user. Offer comparison must be legible on a phone screen without horizontal scrolling. Live Requests hard-expire in 48 h. |
| **Mobile app — Vendor mode** | iOS / Android (shared dual-mode binary) | Optimised for frequent, working-tool use. Non-`ACTIVE` accounts see Awaiting Approval only. Information density is prioritised over whitespace. The Request feed must support rapid triage — scan, filter, open, offer — with the Submit Offer action reachable in at most two taps from the feed. |
| **Admin Portal** | Responsive web, ≥ 1280 px | Optimised for queue processing. Verification, hold-for-approval review moderation, and abuse queues must support keyboard-driven review without leaving the queue context. |
...
> **Sign-off progress (v1.1):** §9.5 commercial decisions and the §1–3 / §5 / §9 PO walk are **resolved**. Remaining: Legal review of Yahoo Finance redistribution and ToS liability wording; remaining Appendix B.3 `[ASSUMED]` items not decided in this walk (e.g. media limits, notification edge cases) should still be triaged before build freeze.
