...
/// CUS-S16 — revealed Vendor identity, Talk, close, review, report.
class CustomerConnectionDetailScreen extends ConsumerStatefulWidget {
  const CustomerConnectionDetailScreen({super.key, required this.connectionId});

  final String connectionId;

  @override
  ConsumerState<CustomerConnectionDetailScreen> createState() =>
      _CustomerConnectionDetailScreenState();
}

class _CustomerConnectionDetailScreenState
    extends ConsumerState<CustomerConnectionDetailScreen>
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(customerConnectionDetailProvider(widget.connectionId));
    ...
    return Stack(
      children: [
        Scaffold(
          key: const Key('customer-connection-detail-screen'),
          appBar: AppBar(
          ...
          body: async.when(
            ...
            error: (err, _) => KhErrorView(
              ...
              onRetry: () => ref
                  .read(customerConnectionDetailProvider(widget.connectionId).notifier)
                  .reload(),
...
class _Body extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final notifier =
        ref.read(customerConnectionDetailProvider(connection.id).notifier);
