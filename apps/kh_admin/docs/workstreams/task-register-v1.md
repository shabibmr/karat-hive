...
| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S1-01** add `path:` deps _completed: 2026-09-09T06:32:00 (duration: 9m 00s)_ | `ADM-SMP-65`, `ADM-SMP-01`, E15 | `apps/kh_admin/pubspec.yaml` | Add `path:` deps on `packages/kh_core`, `kh_domain`, `kh_api`, `kh_design_system`, `kh_l10n`, `kh_ui_domain` exactly as `apps/kh_mobile/karat_hive/pubspec.yaml` does. | S0 PR1 | `flutter pub get` resolves; admin can `import 'package:kh_core/…'`. |
| `[x]` **TR-S1-02** DECIDE: Melos inclusion slice _decision: follow-up slice after S1_ | `ADM-SMP-65` | `root pubspec.yaml`, `melos.yaml` | Decision recorded: separate follow-up slice after S1 data spine work to prevent monorepo script/dependency churn during active migration. | TR-S1-01 | separate follow-up slice recorded; TR-S1-03 deferred to that slice. |
...
| `[x]` **TR-S1-04** replace `ApiException` fork with `kh_core` `Failure` _completed: 2026-09-09T07:10:00 (duration: 9m 30s)_ | `ADM-SMP-01` | `lib/core/api/api_exception.dart`, all `catch` sites | Swap the flat `ApiException` for `kh_core`'s sealed `Failure` (or a thin adapter). Keep call sites compiling. | TR-S1-01 | `api_exception.dart` is gone or a ≤20-line adapter; no parallel error class. |
| `[x]` **TR-S1-05** replace token-storage fork _completed: 2026-09-09T07:10:00 (duration: 9m 30s)_ | `ADM-SMP-01` | `lib/core/auth/token_storage.dart` | Use `kh_core`'s token storage (typed expiries), or adapt; stop keeping expiries as raw strings. | TR-S1-01 | expiries are `DateTime`; no bespoke parsing in `token_storage.dart`. |
| `[x]` **TR-S1-06** reconcile `KhStatusChip` _completed: 2026-09-09T06:44:00 (duration: 11m 00s)_ | `ADM-SMP-01` | `lib/core/design/widgets/kh_status_chip.dart` | Align the admin `KhStatusChip` (5 tones) with `kh_design_system`'s (6 tones, same class name, incompatible enum) — import the shared one or map onto its enum. | TR-S1-01 | one `KhStatusChip` type in use; enum matches the shared package. |
| `[x]` **TR-S1-07** `MaskedParty` / `RevealedParty` types _completed: 2026-09-09T07:10:00 (duration: 9m 30s)_ | E15, `ADM-INS-06`, `AD-FE-07` | `lib/features/offers/model/**` (`OfferParentRequestSummary`), import from `kh_domain` if it exists | Replace bare `String? customerName / customerMobile / customerEmail` on `OfferParentRequestSummary` with a `MaskedParty` / `RevealedParty` type so a pre-acceptance widget cannot compile against identity fields. | TR-S1-01 | identity fields are a sealed type, not nullable `String`; pre-acceptance widgets reference only the masked variant. |

### Phase 2 — normalise the envelope once

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S1-08** fix double-wrap in `_send` _completed: 2026-09-09T07:14:00 (duration: 4m 00s)_ | `ADM-SMP-62`, `ADM-SMP-04` | `lib/core/api/api_client.dart` | Apply the same pre-`ADM-C-70` double-wrap unwrap that `getCollection` does for lists, inside `ApiClient._send`, so single-entity responses are unwrapped centrally too. | Phase 1 | a single-entity GET returns the inner object with no per-repo help. |
| `[x]` **TR-S1-09** shared single-entity unwrap _completed: 2026-09-09T07:14:00 (duration: 4m 00s)_ | `ADM-SMP-05` | `lib/core/api/api_client.dart` or `lib/core/api/json_parse.dart` | Add one `unwrapEntity` path; remove the need for repo-invented sentinel fields (`id`/`profile`/`user`, `titleEn`, `key`, `name`). | TR-S1-08 | one function knows the single-entity shape. |
| `[x]` **TR-S1-10a..o** delete per-repo sentinel + hand-flatten _completed: 2026-09-09T07:14:00 (duration: 4m 00s)_ | `ADM-SMP-05`, `ADM-SMP-10` | one row per repo: `lib/features/{vendors,requests,offers,customers,connections,audit,abuse,moderation,announcements,admin_users,verification,taxonomy,reports,dashboard,settings}/repository/*.dart` | Per repo: delete its bespoke envelope-shape heuristic and its manual `user`/`profile` flattening; call the shared unwrap from TR-S1-09. | TR-S1-09 | that repo contains no envelope-shape or flatten heuristic. |
| `[x]` **TR-S1-11** route bypassing repos through `getCollection` _completed: 2026-09-09T07:14:00 (duration: 4m 00s)_ | `ADM-SMP-62` | `lib/features/verification/repository/*.dart`, `lib/features/taxonomy/repository/*.dart`, `lib/features/reports/repository/*.dart` | Make these 3 repos call `ApiClient.getCollection` instead of hand-rolling the list fetch/unwrap. | TR-S1-09 | none of the 3 build a list from a raw `_send`. |
| `[x]` **TR-S1-12** envelope regression test _completed: 2026-09-09T07:14:00 (duration: 4m 00s)_ | `ADM-SMP-62` | `test/core/api/api_client_test.dart` | Add cases: double-wrapped list, double-wrapped entity, nested `user`/`profile`, all unwrap through the shared path. | TR-S1-08..11 | tests cover all four shapes. |

### Phase 3 — the list kernel

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S1-13** `Paginated<T>` envelope _completed: 2026-09-09T07:22:00_ | `ADM-SMP-25` | `[NEW] lib/core/list/paginated.dart` | One generic `Paginated<T>` (items + `nextCursor` + total?) replacing the 11 `XxxListPage` DTOs. | Phase 2 | type exists + tested; `hasMore` is a getter on `nextCursor != null`, not a stored field. |
| `[x]` **TR-S1-14** kernel state type _completed: 2026-09-09T07:22:00_ | E9, `ADM-INS-19` | `[NEW] lib/core/list/list_state.dart` | Sealed state (or `AsyncValue<Paginated<T>>` + pagination meta) where `loading && error` is **unrepresentable**. No `isLoading`/`isLoadingMore`/`error`/`items` bag. | TR-S1-13 | `loading + error` cannot be constructed; state is exhaustively switched. |
| `[x]` **TR-S1-15** `CursorPaginatedNotifier<TItem,TFilters>` _completed: 2026-09-09T07:22:00_ | E9, `ADM-SMP-61`, `-20`, `-21` | `[NEW] lib/core/list/cursor_paginated_notifier.dart` | One `AsyncNotifier`-based kernel: `build()` returns the first fetch (folds in the six controllers' `Future.microtask(refresh)`), `nextPage()`, `applyFilters()`, `retry()`. Adopt `kh_core/src/paged_list_controller.dart` (cursor-cycle detection, dedup-by-key) where it fits. Enforce `limit` ≤ 100. Every catch uses `TR-S0-19`'s helper (`on ApiException`, never `e.toString()`). | TR-S1-14, TR-S0-19..22 | kernel unit-tested: first load, next page, filter change resets cursor, retry after error, cursor cycle halts. |
| `[x]` **TR-S1-16** kernel exclusive-state test _completed: 2026-09-09T07:22:00_ | E9, `ADM-INS-19` | `test/core/list/cursor_paginated_notifier_test.dart` | Loading→error→retry→loaded transition test; assert no impossible combos. | TR-S1-15 | test passes; cross-linked from `TR-S7-08`. |
| `[x]` **TR-S1-17a..j** migrate each list feature onto the kernel _completed: 2026-09-09T07:34:00_ | E9, `ADM-SMP-61` | one row per feature — Family A: `vendors`, `requests`, `offers`, `customers`, `connections`, `audit`; Family B: `abuse`, `moderation`, `announcements`, `admin_users`. Files: `lib/features/<f>/controller/*_list_controller.dart` (+ `.freezed.dart`), `lib/features/<f>/model/*_page.dart` | Per feature: delete the bespoke controller + page DTO, re-express on `CursorPaginatedNotifier` + `Paginated<T>`; Family B gains cursor+prev/next it lacked. No screen-layout change. | TR-S1-15 | that feature's list loads/pages/filters via the kernel; its old `*ListPage` and `hasMore` field are deleted; feature tests green. |

### Phase 4 — filter models

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S1-18** pick the filter idiom _completed: 2026-09-09T07:41:00_ | `ADM-SMP-22`, `-23` | — (decision, record here) | Standardized value equality (`==`/`hashCode`) + uniform explicit clear semantics (`clearField = false` for hand-written models, and `@freezed` with nullable replacement for generated models). Dropped disparate `Function()?` callback idiom. | Phase 3 | idiom written in this row. |
| `[x]` **TR-S1-19a..k** convert each filter model _completed: 2026-09-09T07:41:00_ | `ADM-SMP-22`, `-23` | one row per model: `lib/features/{vendors,requests,offers,customers,connections,audit,abuse,moderation,announcements,admin_users,reports}/model/*_filters.dart` | Value equality implemented across all 11 filter models: `@freezed` models (`vendors`, `requests`, `offers`) have generated equality; `customers` dropped `T? Function()?` and gained `clearAccountState`; `abuse`, `moderation`, `announcements`, and `reports` gained `==` and `hashCode`. | TR-S1-18 | that model is value-equal and clears the one agreed way. |
| `[x]` **TR-S1-20** rebuild-skip check _completed: 2026-09-09T07:41:00_ | `ADM-SMP-23` | `test/core/list/filter_rebuild_skip_test.dart` | Unit tests verify that `CursorPaginatedNotifier.applyFilters` skips refetch when filter is identical, across customer, abuse, moderation, announcement, and report models. | TR-S1-19* | a re-applied identical filter is a no-op. |

### Phase 5 — query-params codec + URL state

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S1-21** `QueryParamsCodec<T>` _completed: 2026-09-09T07:48:00_ | E19, `ADM-SMP-63` | `[NEW] lib/core/router/query_params_codec.dart` | One codec: encode/decode filters **+ cursor + selected id** to/from query params. Reuse `applyQueryParameters` + `stringMapsEqual` from `lib/core/router/query_navigation.dart`. | Phase 4 | codec round-trips a full list state; unit-tested. |
| `[x]` **TR-S1-22** fix `Uri.replace` clear bug — taxonomy _completed: 2026-09-09T07:48:00_ | `ADM-SMP-64` | `lib/core/router/taxonomy_query_params.dart` (`:66`) | Migrate onto `query_navigation.dart`'s shared extension (which passes `const <String,String>{}`), so clearing the last filter clears the query string. | TR-S1-21 | clearing the last taxonomy filter empties the URL query. |
| `[x]` **TR-S1-23** fix `Uri.replace` clear bug — verification _completed: 2026-09-09T07:48:00_ | `ADM-SMP-64` | `lib/core/router/verification_query_params.dart` (`:54`) | Same migration as TR-S1-22. | TR-S1-21 | clearing the last verification filter empties the URL query. |
| `[x]` **TR-S1-24** clear-semantics regression test _completed: 2026-09-09T07:48:00_ | `ADM-SMP-64` | `test/core/router/query_navigation_test.dart` | Test: apply filter → clear → URL query is empty, for the shared extension. | TR-S1-22, -23 | test passes. |
| `[x]` **TR-S1-25a..e** finish URL migration on the 5 unmigrated files _completed: 2026-09-09T07:53:00_ | `ADM-SMP-07`, E19 | one row each: the 5 `*_query_params.dart` not yet on `applyQueryParameters` (`vendor`, `request`, `offer` + the 2 above are the started set; enumerate the actual 5 unmigrated at pickup from `grep -L applyQueryParameters lib/core/router/*_query_params.dart`) | Per file: move to the shared nav extension + `QueryParamsCodec`. | TR-S1-21 | that file uses the shared extension; no bespoke `Uri.replace`. |
| `[x]` **TR-S1-26a..h** add URL state to the 8 list screens that have none _completed: 2026-09-09T07:58:00_ | `ADM-SMP-63`, `ADM-SMP-07` | one row per screen: `customers`, `connections`, `audit`, `abuse`, `moderation`, `announcements`, `admin_users`, plus 1 more (the review says **8 of 13** lack URL state — confirm the 8th at pickup) — files `lib/features/<f>/presentation/*_list_screen.dart` + a new `<f>_query_params.dart` | Per screen: wire `QueryParamsCodec` so filters + cursor + selected id round-trip through the URL. | TR-S1-21, TR-S1-17* for that feature | deep-linking that screen's URL restores its full state. |

### Phase 6 — server-side filters + audit call

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S1-27** push request-list filters to the API _completed: 2026-09-09T08:02:00_ | E20, `ADM-INS-46` | `lib/features/requests/repository/request_repository.dart` | Send filters as query params; stop the post-fetch `where`. Coordinate any missing param with `docs/admin-backend-api-gaps.md`. | Phase 3 | no client-side `where` after fetch; `hasMore` follows the filtered cursor. |
| `[x]` **TR-S1-28** give `admin_users` a cursor _completed: 2026-09-09T08:04:00_ | `ADM-SMP-29` | `lib/features/admin_users/repository/admin_user_repository.dart`, controller | Stop fetching the whole dataset; use cursor pagination like the others. | TR-S1-17 (admin_users) | `admin_users` requests one page at a time. |
| `[x]` **TR-S1-29** audit any other page-local `where` _completed: 2026-09-09T08:13:00_ | E20 | `grep -rn "\.where(" lib/features/*/repository lib/features/*/controller` | Remove any remaining post-fetch filtering; push to API. | TR-S1-27 | no repo/controller filters a fetched page in memory. |
| `[x]` **TR-S1-30** customer-list access audit call _completed: 2026-09-09T08:14:30_ | E30, `ADM-INS-40`, `FR-ADM-010` AC5 | `lib/features/customers/repository/customer_repository.dart` or controller | On customer-list load, POST the access-audit entry (mirrors the detail-view audit note). | TR-S1-17 (customers) | opening the customer list writes one audit entry. |
| `[x]` **TR-S1-31** backend follow-up log _completed: 2026-09-09T08:15:00_ | E20 | `docs/admin-backend-api-gaps.md` | Record every filter param the API still can't accept. | TR-S1-27..29 | each gap has a row in that doc. |
...
| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S2-01** `KhDetailRow` _completed: 2026-09-09T06:27:15 (duration: 9m 15s)_ | `ADM-SMP-11` | `[NEW] lib/core/design/widgets/kh_detail_row.dart` | One label/value row widget replacing `_buildDetailRow` (request), `_DetailRow` (offer), `_buildFieldRow` (vendor `:260`), `_buildInfoRow` (customer `:330`), inline (connection). | S0 | widget exists, unit-tested; `TR-S7-11` golden requested. |
| `[x]` **TR-S2-02** `KhDetailCard` _completed: 2026-09-09T06:27:00 (duration: 9m 00s)_ | `ADM-SMP-12` | `[NEW] lib/core/design/widgets/kh_detail_card.dart` | One card scaffold replacing the `Container`+`BoxDecoration` literal (8× in `request_detail_screen.dart` alone). | S0 | widget exists, unit-tested; golden requested. |
| `[x]` **TR-S2-03 DECIDE** feedback-banner visual winner _decision: vendor-detail treatment wins — `padding: EdgeInsets.all(kh.spacing.md)`, `kh.shapes.roundedMd`, fill alpha 0.12, border tone alpha 0.4 (softer of the two — 2/3 sites used it), outline icon (`check_circle_outline`/`error_outline`) size 20, `kh.typography.bodySmall` in tone colour `FontWeight.w600`, dismiss shown only when `onDismiss != null`_ _completed: 2026-09-09T09:05:00_ | `ADM-SMP-08` | screenshots of `customer_detail:222`, `request_detail:248`, `vendor_detail:178` | Pick the winning padding / alpha (0.10 vs 0.12) / border / icon style (outline vs filled) / weight / dismissibility. Record the choice. | — | one visual spec written here; design sign-off noted. |
| `[x]` **TR-S2-04** `KhFeedbackBanner` _completed: 2026-09-09T09:05:00_ | `ADM-SMP-08` | `[NEW] lib/core/design/widgets/kh_feedback_banner.dart`, the 3 call sites | Build to the TR-S2-03 spec; **preserve each test-facing `Key`**; swap all 3 call sites. | TR-S2-03 | one banner widget; all 3 `Key`s intact; screens visually match the chosen winner. — widget + `test/core/design/widgets/kh_feedback_banner_test.dart` (6 cases); call sites pass `key:` through; the 3 `_buildFeedbackBanner` methods deleted; `flutter analyze` clean. |
| `[x]` **TR-S2-05** replace `_buildStatTile` with `KhMetricCard` _completed: 2026-09-09T06:40:00 (duration: 7m 00s)_ | `ADM-SMP-09` | `lib/features/announcements/presentation/announcements_screen.dart` (`:312`) | Delete the hand-rolled metric tile; use the existing `KhMetricCard`. | S0 | no `_buildStatTile` in the file; tile renders via `KhMetricCard`. |
...
| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S2-15** move `fetchDocumentUrl` into the controller _completed: 2026-09-09T09:35:00_ | E17, `ADM-INS-11` | `lib/features/verification/presentation/verification_detail_pane.dart`, `lib/features/verification/controller/verification_controller.dart` | Move the `ref.read(verificationRepositoryProvider).fetchDocumentUrl` call into `VerificationController`; the pane reads controller state. | S0 | grep `ref.read(*RepositoryProvider)` in any `presentation/` file → 0. — new `VerificationDocViewController` (`FamilyNotifier<VerificationDocView,String>`) owns `openDocument()` + loading/error/opened state and the `_resolveDocumentUrl` base-prefix; pane watches `verificationDocViewControllerProvider`, resolves the raw error via `resolveApiErrorMessage`, catch is `on ApiException`. Verification tests green. |
| `[x]` **TR-S2-16** sweep for other presentation→repo calls _completed: 2026-09-09T09:35:00_ | E17 | `grep -rn "RepositoryProvider" lib/features/*/presentation` | Fix any other hit the same way. | — | zero repo reads in `presentation/`. — grep `RepositoryProvider` under `lib/features/*/presentation/**` → 0 after TR-S2-15 (it was the only hit). |
...
| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S3-01** virtualise `KhDataTable` body _completed: 2026-09-09T06:31:40 (duration: 13m 40s)_ | E13, `ADM-SMP-40` | `lib/core/design/widgets/kh_data_table.dart` (`:73-81`) | Replace the `Column` + `for` row build with a vertical `ListView.builder` (fixed `itemExtent` from `kh.spacing.tableRowHeight`) inside the existing horizontal scroller. Keep row hover (`InkWell.hoverColor`) + header behaviour. **API unchanged.** | S0 | a 500-row page builds a bounded number of row widgets. |
| `[x]` **TR-S3-02** table golden behind old API _completed: 2026-09-09T11:32:00 (duration: 1m 00s)_ | E13, `ADM-SMP-40` | `test/core/design/widgets/kh_data_table_golden_test.dart` | Golden the virtualised table (LTR / RTL / 200% text) before any screen rewire. | TR-S3-01 | goldens committed; cross-linked from `TR-S7-10`. — `test/core/design/widgets/kh_data_table_golden_test.dart` covers LTR, RTL, and 200% text scale; 3 goldens generated and verified green. |
| `[x]` **TR-S3-03a..l** drop outer scroll + rewire per list screen _completed: 2026-09-09T12:20:00 (duration: 48m 00s)_ | E13, `ADM-SMP-41` | one row per screen: `request_list`, `vendor_list`, `offer_list`, `announcements`, `platform_settings`, `admin_users`, `moderation`, `abuse`, `audit`, `connection_list`, `customer_list`, `reports` — `lib/features/<f>/presentation/*_list_screen.dart` (+ `reports` presentation) | Per screen: remove the outer `SingleChildScrollView` wrapping `KhDataTable`; let the table own the one scroll view. | TR-S3-01 **and** S1 migrated that feature (`TR-S1-17*`) | that screen has exactly one scroll view for the list; no nested scroll/layout pass. — all 12 list screens refactored to remove outer SingleChildScrollView and wrap lazy KhDataTable in Expanded. |
| `[x]` **TR-S3-04a..l** `.select()` granularity per list screen _completed: 2026-09-09T12:20:00 (duration: 48m 00s)_ | `ADM-SMP-42`, `ADM-INS-73` | same 12 screens as TR-S3-03 | Per screen: `ref.watch(listProvider.select((s) => s.items))` for the table; separate watches for filters, pagination flags, error. | TR-S1-14 (clean selectors) **and** TR-S3-03 for that screen | toggling a filter chip or the "loading more" flag does not rebuild the row list. — fine-grained selectors and ConsumerWidgets wired across all 12 screens. |
| `[x]` **TR-S3-05** image decode bounds _completed: 2026-09-09T06:49:00 (duration: 16m)_ | E29, `ADM-INS-72` | KYC / vendor document thumbnail widgets (`lib/features/{vendors,verification}/presentation`) | Audited vendor & verification features: zero inline thumbnail decode widgets exist; documents open externally via `openUrlInNewTab`; verified with test (`verification_image_decode_bounds_test.dart`) that zero uncontrolled `Image` decodes occur. | S0 | zero uncontrolled image decodes across KYC documents. |
| `[x]` **TR-S3-06** hand off signed-URL KYC open to S4 _completed: 2026-09-09T09:35:00_ | E29, `ADM-INS-48` | note only → `TR-S4-*` (media/auth path) | S3 bounds decode only; the bearer-less `/v1/media/<key>` → signed-URL change is S4's. Record the dependency. | — | a row in S4 (`TR-S4-19`) owns the signed-URL open. — recorded in `docs/admin-backend-api-gaps.md` GAP-ADM-07: S3 owns decode bounds only (`TR-S3-05`); the presigned/public media-URL open is owned by S4 (`TR-S4-19`); admin client already prefixes `khApiBase` + opens externally (`TR-S2-15`) and will render once the gap closes. |
...
| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S4-01** web token policy — access token in memory _completed: 2026-09-09T08:00:00 (duration: 6m 00s)_ | E1, `ADM-INS-09` | `lib/core/auth/token_storage.dart` | On `kIsWeb`, keep the access token **in memory only** (no `FlutterSecureStorage`, no `WebOptions`). | S0, TR-S1-05 | dev-tools shows no access token in web storage. |
| `[x]` **TR-S4-02** web token policy — no persisted refresh token _completed: 2026-09-09T08:00:00 (duration: 6m 00s)_ | E1 | `lib/core/auth/token_storage.dart`, `lib/core/auth/session_controller.dart` | On web, do not persist the refresh token; re-auth on reload (silent refresh or login screen). | TR-S4-01 | a reload triggers silent re-auth or shows login; nothing in web storage. |
| `[x]` **TR-S4-03** idle timeout timer _completed: 2026-09-09T06:32:38 (duration: 14m 38s)_ | E2, `ADM-INS-30`, `FR-ADM-001` AC4 | `[NEW] lib/core/auth/idle_timeout.dart`, `test/core/auth/idle_timeout_test.dart` | 60-minute idle timer; pointer/keyboard activity resets it. | S0 | 60 min idle fires the timeout callback; activity resets the clock. |
| `[x]` **TR-S4-04** idle warning dialog + logout _completed: 2026-09-09T06:45:51 (duration: 12m 51s)_ | E2 | `lib/core/auth/idle_timeout.dart`, a warning dialog widget | Warn before logout; on no response, log out. | TR-S4-03 | idle → warning dialog → logout. |
| `[x]` **TR-S4-05** multi-tab logout broadcast _completed: 2026-09-09T08:01:00 (duration: 8m 00s)_ | `ADM-INS-85` | `lib/core/auth/session_controller.dart`, web broadcast channel | Broadcast logout across tabs (don't wait for a 401). | TR-S4-01 | logging out in one tab logs out the others. |

### Login gating

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S4-06** gate the password form _completed: 2026-09-09T07:50:00 (duration: 59m 00s)_ | E7, `ADM-INS-31`, `adr/0010` | `lib/features/auth/presentation/**` | Show the email/password form **only** when `KH_DEV_AUTOLOGIN` is set; production path is Google Sign-In. | S0 | a prod-flavour build shows only "Sign in with Google". |
| `[x]` **TR-S4-07** remove seed password default _completed: 2026-09-09T07:50:00 (duration: 59m 00s)_ | E7, `ADM-INS-45` | `lib/core/auth/dev_auth.dart` | Delete the `'AdminSecret123!'` default (or move behind `KH_DEV_AUTOLOGIN`). | TR-S4-06 | grep `AdminSecret123` in `lib/` → 0. |

### Gold-rate + empty shells

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S4-08 DECIDE** delete empty feature dirs _decision: confirmed delete (ADM-S20 deferred)_ | E8, `ADM-SMP-31` | `lib/features/gold_rate/**`, `lib/features/auth/{controller,model,repository}/**` (`.gitkeep`-only) | Confirm deletion of the `.gitkeep`-only shells (ADM-S20 deferred). Record. | — | decision noted here. |
| `[x]` **TR-S4-09** hide the Gold Rates nav entry _completed: 2026-09-09T07:50:00 (duration: 59m 00s)_ | E8, `ADM-INS-21` | `lib/core/shell/kh_admin_scaffold.dart`, `lib/core/router/app_router.dart` | Remove/hide the `/gold-rates` nav item + placeholder route until ADM-S20. | — | no placeholder screen reachable. |
| `[x]` **TR-S4-10** delete empty feature dirs _completed: 2026-09-09T07:50:00 (duration: 59m 00s)_ | E8 | `lib/features/gold_rate/**`, empty `lib/features/auth/*` dirs | Delete per TR-S4-08. | TR-S4-08 = "delete" | `lib/features/` has no `.gitkeep`-only dir. |

### Bundle

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S4-11** `deferred as` for heavy routes _completed: 2026-09-09T08:02:30 (duration: 9m 30s)_ | E23, `ADM-INS-37` | `lib/core/router/app_router.dart` | Load `reports`, `audit`, `announcements` screens via `deferred as`. | S0 | `flutter build web` no longer bundles `fl_chart` in the main chunk. |
| `[x]` **TR-S4-12** drop `cloud_firestore` _completed: 2026-09-09T07:50:00 (duration: 59m 00s)_ | E23, `ADM-SMP-30`, `AD-FE-09`, `C-12` | `apps/kh_admin/pubspec.yaml` | Remove `cloud_firestore` (service already deleted in Tier-0). | TR-S1-01 | package gone; `flutter pub get` clean; no Firestore import in `lib/`. |
| `[x]` **TR-S4-13** drop `firebase_analytics` _completed: 2026-09-09T07:50:00 (duration: 59m 00s)_ | E23, `ADM-INS-41` | `apps/kh_admin/pubspec.yaml`, any `FirebaseAnalyticsObserver` refs | Remove `firebase_analytics` (observer never attached). | TR-S1-01 | package gone; no analytics import. |
| `[x]` **TR-S4-14** drop `cupertino_icons` _completed: 2026-09-09T07:50:00 (duration: 59m 00s)_ | E23, `ADM-INS-42` | `apps/kh_admin/pubspec.yaml` | Remove unused `cupertino_icons`. | TR-S1-01 | package gone; build clean. |

### Flavours

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S4-15** flavour provider graph _completed: 2026-09-09T08:05:00 (duration: 11m 00s)_ | E26, `ADM-INS-81` | `[NEW] lib/core/platform/flavor.dart`, `lib/main.dart` | `dev` / `staging` / `prod` via `--dart-define-from-file`; per-flavour provider overrides. | S0 | each flavour boots with its own config. |
| `[x]` **TR-S4-16** prod refuses non-HTTPS base _completed: 2026-09-09T08:05:00 (duration: 11m 00s)_ | E26, `ADM-INS-82` | `lib/core/platform/flavor.dart` or `lib/core/api/api_client.dart` | Prod flavour throws at startup if `KH_API_BASE` is not `https://`. | TR-S4-15 | a `prod` build against `http://` fails fast with a clear message. |
| `[x]` **TR-S4-17** contract-version / 426 screen _completed: 2026-09-09T08:05:00 (duration: 11m 00s)_ | E26 | `[NEW] lib/features/…/contract_mismatch_screen.dart`, `lib/core/api/api_client.dart` | On a contract-version mismatch / HTTP 426, show a dedicated screen. | TR-S4-15 | a simulated 426 renders the screen, not a generic error. |

### FCM + Firebase init

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S4-18** wire the FCM foreground handler (or poll) _completed: 2026-09-09T08:08:45 (duration: 12m 45s)_ | E27, `ADM-INS-08`, `ADM-SMP-46` | `lib/core/firebase/firebase_notification_service.dart`, `lib/main.dart` | Call `initializeForegroundHandler` (or a 30s poll) to invalidate list providers on a relevant push. **Coordinate provider names with S1's kernel.** | TR-S1-15 (provider names) | an incoming push (or the poll) refreshes an open list. |
| `[x]` **TR-S4-19** non-blocking Firebase init, fail closed _completed: 2026-09-09T08:08:00 (duration: 12m 00s)_ | E27, `ADM-INS-86` | `lib/main.dart` (`:16`) | Make `Firebase.initializeApp` not block the first frame; if Google Sign-In is the only prod login, an init failure must **block login**, not continue silently. | TR-S4-06 | a simulated Firebase init failure blocks login in prod. |
| `[x]` **TR-S4-20** decide `firebase_messaging` keep/drop _decision: keep firebase_messaging (foreground handler wired to FCM push in TR-S4-18)_ _completed: 2026-09-09T08:08:45 (duration: 5m 00s)_ | E23, E27 | `apps/kh_admin/pubspec.yaml` | If TR-S4-18 chose polling, drop `firebase_messaging`; else keep. Record. | TR-S4-18 | package state matches the TR-S4-18 choice. |

### Routing + platform

| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S4-21** `AdminRoutes` constants _completed: 2026-09-09T07:50:00 (duration: 59m 00s)_ | `ADM-INS-04` | `[NEW] lib/core/router/admin_routes.dart`, `lib/core/router/app_router.dart`, `lib/core/shell/kh_admin_scaffold.dart` | One constants class; replace the magic route strings duplicated between `kAdminNavItems` and the `GoRoute` table. | S0 | no literal route string outside `AdminRoutes`. |
| `[x]` **TR-S4-22** reject empty `:id` _completed: 2026-09-09T07:50:00 (duration: 59m 00s)_ | `ADM-INS-33` | `lib/core/router/app_router.dart` | A route with an empty `:id` (`state.pathParameters['id'] ?? ''`) redirects to the list. | TR-S4-21 | `/vendors//` does not render a broken detail screen. |
| `[x]` **TR-S4-23** `dart:html` → `package:web` _completed: 2026-09-09T06:54:00 (duration: 3m 00s)_ | `ADM-INS-17` | `lib/core/platform/open_url_web.dart` | Replace `dart:html` with `package:web` (or `url_launcher`). | S0 | `avoid_web_libraries_in_flutter` + `deprecated_member_use` clear; URL open still works. |
...
| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S6-01** per-queue-source state on the dashboard _completed: 2026-09-09T07:20:00_ | E4, `ADM-INS-61` | `lib/features/dashboard/controller/dashboard_controller.dart`, `lib/features/dashboard/presentation/**` | Each queue source carries its own `AsyncValue`; remove the `catch`-returns-`[]` swallow. | S0 | a queue source that throws is `error`, not empty. — `dashboardQueueSourceProvider` = `FutureProvider.family` per `DashboardQueueKind`, no catch; `_omitQueueSource` deleted. |
| `[x]` **TR-S6-02** per-source error chip + retry _completed: 2026-09-09T07:20:00_ | E4 | `lib/features/dashboard/presentation/**` | Render a per-source error chip with a **Retry**, not a vanished section; message via `TR-S0-19` helper. | TR-S6-01, `TR-S0-19` | killing the verification endpoint shows an error chip on that queue, not "0 pending". — `_QueueSourceErrorChip` keyed `dashboard-queue-source-error-<kind>`, retry invalidates that source; message via `resolveApiErrorMessage`. Same pattern applied to the trend section. |
| `[x]` **TR-S6-03** dashboard date-range selector _completed: 2026-09-09T07:20:00_ | E21, `ADM-INS-32` | `lib/features/dashboard/**` | Add a date-range control feeding the queries. | S0 | changing the range refetches. — `DashboardRange` (7/30/90d) `StateProvider`; `fetchStats({from,to})` + `fetchTrend` re-run on change. Backend ignores the params today → GAP-ADM-09 logged. |
| `[x]` **TR-S6-04** dashboard trend series _completed: 2026-09-09T07:20:00_ | E21 | `lib/features/dashboard/**`, `fl_chart` | Show a trend series for the key metrics. | TR-S6-03 | a trend renders for the selected range. — "Trends" section reuses `ReportChart` fed by range-scoped `GET /v1/admin/reports/request-volume`. No time-series endpoint exists → GAP-ADM-10 logged. |
...
| `[x]` **TR-S6-06 DECIDE** abuse action set _decision: leave at current options (Warn, Suspend, Deactivate) per FR-ADM-032_ _completed: 2026-09-09T12:31:00_ | E22, `FR-ADM-032` (`[ASSUMED]`) | — | Confirm the warn / suspend / deactivate set against `FR-ADM-032`; Product sign-off. Record. | — | action set confirmed as Warn, Suspend, Deactivate. |
...
repo-wide `docs/`.
**Depends on:** — · **Blocks:** — · **Parallelism:** fully independent, no code overlap.
...
| Task | Src | Files | Action | Deps | Done when |
|---|---|---|---|---|---|
| `[x]` **TR-S8-01 DECIDE** `SDC-07` move vs copy _decision: Move (surface-only docs moved with redirect stub)_ _completed: 2026-09-09T12:31:00_ | `SDC-07` | — | Confirm: surface-only docs referenced by nothing else are **moved** with a redirect stub (verified-safe list: 4 admin docs, vendor/customer task docs, 7 backend docs). Record. | — | decision confirmed as Move with redirect stub. |
| `[x]` **TR-S8-02 DECIDE** `SDC-08` skill timing _decision: No (skill authored in parallel / without waiting for after pilot)_ _completed: 2026-09-09T12:31:00_ | `SDC-08` | — | Confirm whether the carve skill is authored after the admin pilot. Record. | — | decision confirmed: do not block skill on pilot. |
