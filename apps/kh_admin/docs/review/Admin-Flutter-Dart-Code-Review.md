# Flutter/Dart code review — Admin Portal
...
| **Checklist** | Library-agnostic Flutter/Dart review (widget, Riverpod, performance, a11y, security, l10n, DI, analysis) |
| **Status** | Snapshot. **Not the plan of record.** Does not override SRS v1.3, Architecture-Frontend, or `Admin-App-Completion-Plan.md`. |
| **Does not mint** | `FR-*`, `AD-FE-*`, `ADM-S*`, `SAM-GAP-*`, `ADM-INS-*` — those are cited, not replaced. |
| **Companion** | Spec/architecture findings already numbered in [`apps/kh_admin/docs/inspection/REGISTER.md`](../apps/kh_admin/docs/inspection/REGISTER.md) (`ADM-INS-01`–`87`). This document maps the **Flutter/Dart engineering** gaps and the enhancements that close them. |
...
| No `print()` in `lib/` | `debugPrint` only (still unfiltered — see §9) |
...
## 3. Findings by checklist section
...
**Enhance:** split each god screen into header / summary / timeline / actions / dialogs widgets. Extract `KhDataTable` rows to a lazy viewport (see §3.5). Ban raw `Colors.*` and raw `TextStyle` via lint + review. Prefer `context.kh.colors.onPrimary` for button foregrounds.
...
| Export poll 400 ms × 40 | `ReportsRepository.pollInterval` | `ADM-INS-71` |
...
### 3.8 Platform and responsive design
...
### 3.10 Packages and dependencies
...
Firestore on a product whose constraint is **no secondary datastore** (`C-12`, `AD-FE-09`) is a policy smell, not just bundle weight.
...
- Leftover `...kAdminNavItems.where(not registered) → _GenericPlaceholderScreen` exists only for gold rates; the exclusion list is a second source of truth.
...
- `unused_element: ignore` hides dead code

A workspace `flutter analyze` (this session) reported **two kh_admin test warnings** and no `lib/` issues — because the config is too loose to see the problems in this review.
...
If only one PR follows this review, do **E3 + E5 + E10 + E18**: typed API errors in list controllers, global Flutter error hooks, strict analyzer (even if it takes a follow-up to clear new lints), and the missing request-list widget test. That slice does not change product scope, does not fight `AD-FE-12` (still `[BLOCKED]`), and makes every later screen inherit a safer default.
...
## 6. Crosswalk to the inspection register
