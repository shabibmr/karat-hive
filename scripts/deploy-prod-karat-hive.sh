#!/usr/bin/env bash
# Deploy Karat Hive Customer / Vendor Web App to Production (/var/www/html/karat/karat-hive)
# Serves: https://app.karathive.com and https://karathive.com
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_DIR="${REPO_ROOT}/apps/kh_mobile/karat_hive"
DEPLOY_DIR="${DEPLOY_DEST:-/var/www/html/karat/karat-hive}"
...
Build and deploy Karat Hive Web App to production (${DEPLOY_DIR}):
  Public URLs: ${PUBLIC_URL}, ${PUBLIC_URL_ALT}
...
echo "============================================================"
echo " Karat Hive Web App Production Deployment"
echo "============================================================"
echo " Repo        : ${REPO_ROOT}"
...
if [[ "${DO_BUILD}" -eq 1 ]]; then
  log "Building karat_hive for web (base-href=${BASE_HREF})"
  cd "${APP_DIR}"
  export BASE_HREF
  export KH_API_BASE
...
if [[ "${DO_DEPLOY}" -eq 1 ]]; then
  ...
  if [[ -n "$(ls -A "${DEPLOY_DIR}" 2>/dev/null || true)" ]]; then
    log "Backing up current deployment → ${BACKUP_ROOT}/karat_hive_${TS}"
    cp -a "${DEPLOY_DIR}" "${BACKUP_ROOT}/karat_hive_${TS}"
  fi
  ...
  CODE1="$(http_code "${PUBLIC_URL}")"
  log "${PUBLIC_URL} → HTTP ${CODE1}"
  [[ "${CODE1}" == "200" ]] || die "Health check failed for ${PUBLIC_URL} (HTTP ${CODE1})"

  CODE2="$(http_code "${PUBLIC_URL_ALT}")"
  log "${PUBLIC_URL_ALT} → HTTP ${CODE2}"
  [[ "${CODE2}" == "200" ]] || die "Health check failed for ${PUBLIC_URL_ALT} (HTTP ${CODE2})"
